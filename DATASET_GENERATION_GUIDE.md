  ...
  "instances": [
    {
      ...
      "surrounding_context": "In the field of machine learning, recent work has shown impressive results [CITATION]. This approach outperforms previous methods by utilizing novel architecture designs. The results demonstrate significant improvements in both accuracy and efficiency across multiple benchmarks [OtherRef2020].",
      "citation_metadata": {
...
- **Citation Marker**: Only the specific citation being processed is replaced with `[CITATION]` marker. Other citations in the same sentence/paragraph remain unchanged (e.g., `[OtherRef2020]` style). This allows the model to focus on one reference at a time.
- **Paragraph Context**: The `surrounding_context` field contains the entire paragraph where the citation appears, providing rich contextual information.
...
- Citation context extraction uses paragraph boundaries (empty lines, `\par`, section headers)
