...
def collect_all_claims_from_paper(directory, references, max_claims=75):
    """
    Forward mapping: scan all .tex files for claim sentences containing citations,
    extract them, validate, and match to references.

    Args:
        directory: Path to extracted source directory
        references: Dict mapping cite_key -> bib_entry
        max_claims: Maximum number of claims to collect per paper (default: 50)

    Returns:
        List of dicts with: claim_text, surrounding_context, cite_key, bib_entry
    """
    tex_files = find_tex_files(directory)
    citation_pattern = r'\\cite[a-z]*(?:\[[^\]]*\])?(?:\[[^\]]*\])?\{([^}]+)\}'
    claims = []
        ...
        try:
            ...
            for line_idx, line in enumerate(lines):
                ...
                for match in matches:
                    ...
                    # Find paragraph boundaries (same logic as extract_citation_context)
                    para_start = line_idx
                    for i in range(line_idx - 1, -1, -1):
                        l = lines[i].strip()
                        if not l or l == '\\par' or l.startswith('\\section') or l.startswith('\\subsection'):
                            para_start = i + 1
                            break
                        para_start = i
                    ...
                    paragraph_text = re.sub(r'\\label\{[^}]+\}', '', paragraph_text)
                    ...
                    claim_marked = claim_sentence.replace(citation_cmd, '[CITATION]')
                    paragraph_marked = paragraph_text.replace(citation_cmd, '[CITATION]')
                    claim_marked = re.sub(r'\s+', ' ', claim_marked).strip()
                    paragraph_marked = re.sub(r'\s+', ' ', paragraph_marked).strip()

                    if not is_valid_claim_text(claim_marked):
                        continue

                    # Create an entry for EACH cite_key that has a bib_entry
                    for cite_key in cite_keys:
                        if len(claims) >= max_claims:
                            break
                        if cite_key in references:
                            claims.append({
                                'claim_text': claim_marked,
                                'surrounding_context': paragraph_marked,
                                'cite_key': cite_key,
            ...
            logging.warning(f"Error processing {tex_file} for claims: {e}")
            continue

    logging.info(f"Collected {len(claims)} valid claims from {len(tex_files)} .tex files")
    return claims


def create_dataset_instance(cite_key, bib_entry, citation_info, arxiv_id=None):
    """
...
def create_dataset_instance_forward(claim, arxiv_id=None):
    """
    Create a dataset instance from a pre-extracted claim (forward mapping).

    Args:
        claim: Dict with claim_text, surrounding_context, cite_key, bib_entry

    Returns:
        Dictionary matching DatasetInstance schema
    """
    metadata = bib_entry_to_metadata(claim['bib_entry'])

    ground_truth = {
        'true_existence': 1,
        'true_hallucination_category': None,
        'true_alignment': 0,
        'expert_rationale': 'Citation from published paper, assumed to be correct and fully supported.'
    }
...
def process_id(arxiv_id, temp_dir, mode='backward'):
    """
    ...
    Args:
        ...
        mode: 'backward' (reference to claim) or 'forward' (claim to reference)
    """
    ...
    try:
        ...
        # Step 4 & 5: Find citations and create dataset instances
        dataset_instances = []
        
        if mode == 'forward':
            logging.info("Step 4 & 5: Forward mapping - collecting claims from paper...")
            claims = collect_all_claims_from_paper(extract_dir, references, max_claims=75)
            
            if not claims:
                logging.warning(f"No valid claims found in .tex files for {arxiv_id}")
                return None
            
            for claim in claims:
                try:
                    instance = create_dataset_instance_forward(claim, arxiv_id)
                    if instance is not None:
                        dataset_instances.append(instance)
                except Exception as e:
                    logging.warning(f"Error creating instance for {claim['cite_key']}: {e}")
                    continue
        else:
            ...
            for cite_key, citation_list in citations.items():
                ...
                for citation_info in citation_list:
                    ...
                    try:
                        instance = create_dataset_instance(cite_key, bib_entry, citation_info, arxiv_id)
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='forward', choices=['backward', 'forward'],
                       help='Mapping mode: backward (reference to claim) or forward (claim to reference)')
    ...
    for i, arxiv_id in enumerate(arxiv_ids, 1):
        ...
        try:
            result = process_id(arxiv_id, temp_dir, mode=args.mode)
