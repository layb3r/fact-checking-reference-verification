"""
Over-Claim Negative Sample Generator for Citation Verification.

Generates "partially supported" adversarial claims by applying only the
OVER_CLAIM semantic drift — taking a modest or specific claim and inflating
it into a universal, absolute, or highly generalized breakthrough.

Uses a Generator → Discriminator → Cosine-Similarity-Filter pipeline.
"""
...
_file_handler = RotatingFileHandler(
    os.path.join(LOGS_DIR, 'over_claim_generator.log'),
...
"""
    ...
    async def _evaluate_candidate(
        ...
        if predicted_label == "SUPPORTED" or predicted_label == "UNSUPPORTED" or predicted_label == "UNCERTAIN":
            return False, f"Failed Judge: Not over-claim", 0.0
        ...
        try:
            ...
            if sim_score < self.similarity_threshold:
                return False, f"Failed Filter: Similarity {sim_score:.2f} < {self.similarity_threshold}", sim_score
                
            return True, predicted_label, sim_score
        except Exception as e:
            logger.warning(f"Embedding failed: {e}")
            return False, "Failed Filter: Embedding Error", 0.0
    ...
    async def generate_hard_negative(
        ...
        for attempt in range(self.max_retries):
            ...
            logger.info(f"Attempt {attempt + 1}/{self.max_retries} | Drift: {drift_type.value} | Judge: {judge_status} | Similarity: {sim_score:.4f}")
    ...
    async def process_batch_over_claim(
        ...
        for inst in instances:
            ...
            if negative_sample:
                neg_inst = json.loads(json.dumps(inst))
                ...
                judge_label = negative_sample["target_alignment_label"]
                neg_inst.setdefault("true_outputs", {})["true_alignment"] = JUDGE_TO_ALIGNMENT.get(judge_label, 2)

                augmented.append(neg_inst)
